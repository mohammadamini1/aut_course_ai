...
## node object
class node:
    def __init__(self, state, parent, previous_action):
        self.state = state
        self.actions = []
        self.children = []
        self.parent = parent
    ...
    def do_action(self, act):
        ...
        return node(new_state, self, act)
...
def bfs(problem):
    ...
    while not frontier.is_empty():
        ...
        if current_node.goal_test():
            return current_node, frontier.queue.__len__(), explored.__len__()
        else:
...
if __name__ == '__main__':
    ...
    problem = node(Input, None, None)
    
    print("\nSearch started!")
    answer_node, frontier_len, explored_len = bfs(problem)
    print("\nSearch finished!")
    import gc
    gc.collect()
    res = answer_node
